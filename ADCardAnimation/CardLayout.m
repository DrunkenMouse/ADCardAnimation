...
@property(nonatomic, assign)CGFloat deltaOffsetY;//每个cell之间的偏移量间距，即第0个cell往下滑动deltaOffsetY个点时会到达第一个cell的位置
...
-(instancetype)initWithOffsetY:(CGFloat)offsetY {
    ...
    if (self = [super init]) {
        ...
        self.m0 = 1000;//移动1000到最顶点
        self.n0 = 250;//第0个Cell的y坐标
        self.deltaOffsetY = 140;//每个cell之间的偏移量间距
    }
    return self;
}
...
//获取当前可以看到的所有控件的LayoutAttributes
...
//如果界面在滚动的过程中,需要修改控件中的大小,就需要将返回值设置为YES
